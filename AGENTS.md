...
A 3D game engine: pure Zig + SDL3 GPU + flecs ECS. Please refer to the repository's [README.md](README.md) for an overview of the project.

## Goals

- **Performance**: pure Zig, fully ECS-based architecture
- **Hackable**: game projects fork and modify the engine repo for maximum flexibility; one language, one type system, one debugger
- **Agent friendly**: simple architecture for easy integration with AI agents
...
- `game/` — user's game project (skeleton: main.zig, components.zig)
...
Each target has its own `components.zig` for game-specific components.

## Details

- Components are plain Zig structs. Tag components are zero-sized structs. All components must be listed in the `all` tuple (in `core_components.zig` for engine components, extended via `withExtra()` in each target's `components.zig`).
- Engine code is split by responsibility: `engine.zig` (lifecycle, registries, frame orchestration, built-in systems), `renderer.zig` (GPU pipeline, scene rendering), `postprocess.zig` (bloom, DoF, composite+tonemap — GPU handles only, settings from Camera component), `debug_server.zig` (HTTP debug server + request queue).
- Adding a new asset handle type (e.g. `TextureHandle` with string name resolution): define the struct, add to the `.all` tuple, add a variant to `HandleKind` in `engine.zig`, and add lookup logic.
...
Global bloom shape: `engine.postprocess.tints[0..6]` (per-level weights) and `engine.postprocess.radius` (upsample filter width).
...
**Architecture:** The HTTP thread accepts connections and queues requests. The main thread drains the queue once per frame (after systems, before rendering). For `/stats`, the HTTP thread blocks until the main thread processes the request. For `/screenshot`, the response is deferred until after the frame renders and the PNG is written.

**MCP server** (`mcp/server.mjs`): Thin Node.js wrapper that exposes the HTTP API as MCP tools (`screenshot`, `get_stats`). Configured in `.mcp.json` for auto-discovery by Claude Code. Install deps with `cd mcp && npm install`.
...
```sh
zig build run               # game (game/main.zig)
zig build run-examples      # examples suite (examples/main.zig)
```
...
**MCP tools**: If the MCP server is running (`mcp/server.mjs`), you also have `screenshot` and `get_stats` available as MCP tool calls.
...
The engine must handle tens of thousands to hundreds of thousands of entities efficiently, even if current demos are small. Design all per-entity paths (queries, iteration, rendering) with this scale in mind. Avoid per-entity allocations or O(n²) patterns in hot loops.
...
- **Document all code thoroughly.** Shader code (GLSL) and Zig engine code must include clear, helpful comments that explain the *why* and *how* — not just the *what*. Assume the reader is a human or AI agent who understands programming but may not know the engine's specific conventions, GPU pipeline details, or rendering math. Include references to techniques and papers where applicable. This codebase should be learnable by reading it.
- Use Conventional Commit formatting for commit messages
- `c` = SDL C namespace (from `engine.zig`). This is a `pub const` export — other files must import `c` from `engine.zig` rather than doing their own `@cImport`.
...
### flecs (ECS) patterns
- The ECS world is `Engine.world: *ecs.world_t`. Import `ecs` via `@import("engine").ecs` or directly as `@import("zflecs")`.
...
- **System dispatch**: All Zig systems are dispatched by flecs `callback` systems (`engineSystemsCallback`). They use `ecs.defer_suspend`/`defer_resume` so mutations apply immediately (systems read-after-write in the same frame). Do NOT use `run` callbacks for zero-term systems — it causes double iterator finalization (flecs issue #905).
...
- **Shared `@cImport`**: Each `@cImport` in Zig creates distinct opaque types. If two files each `@cImport("SDL3/SDL.h")`, their `SDL_GPUDevice` types are incompatible. Always import `c` from `engine.zig`.
